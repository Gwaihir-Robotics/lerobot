...
from .config_mini_mapper import MiniMapperClientConfig


class MiniMapperClient(Robot):
    config_class = MiniMapperClientConfig
    name = "mini_mapper_client"

    def __init__(self, config: MiniMapperClientConfig):
        import zmq
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "MiniMapper Daemon is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for MiniMapper Host to connect expired.")
    ...
    def _get_data(self) -> tuple[dict[str, np.ndarray], dict[str, Any], dict[str, Any]]:
        ...
        If successful, updates and returns the new frames, speed, and state.
    ...
    def get_observation(self) -> dict[str, Any]:
        """
        Capture observations from the remote robot: present wheel speeds 
        (converted to body-frame velocities: x, y, theta), and a camera frame. 
        Receives over ZMQ, translate to body-frame vel
        """
        if not self._is_connected:
            raise DeviceNotConnectedError("MiniMapperClient is not connected. You need to run `robot.connect()`.")
    ...
    def _from_keyboard_to_base_action(self, pressed_keys: np.ndarray):
        ...
        y_cmd = 0.0  # m/s lateral (ignored for differential drive)
        ...
        return {
            "x.vel": x_cmd,
            "y.vel": y_cmd,  # Will be ignored by differential drive
            "theta.vel": theta_cmd,
        }
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command mini_mapper to move with specified velocities. Sends over ZMQ

        Args:
            action (dict): dictionary containing the goal velocities for the robot.
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "MiniMapperClient is not connected. You need to run `robot.connect()`."
            )

        self.zmq_cmd_socket.send_string(json.dumps(action))  # action is in body frame
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "MiniMapper is not connected. You need to run `robot.connect()` before disconnecting."
            )
